  ...
  const handleExport = () => {
    ...
    try {
      ...
      const excelData = filteredData.map((item) => ({
        "ID Article": item.itemId,
        "Nom de l'article": item.itemName || "Non nommé",
        "Quantité en stock": item.quantity?.toFixed(2) || "0.00",
        "Quantité utilisée": item.usedQuantity?.toFixed(2) || "0.00",
        "Total": ((item.quantity || 0) + (item.usedQuantity || 0)).toFixed(2),
        "État du stock": item.quantity === 0 ? "Rupture" : 
                         (item.quantity || 0) < 10 ? "Stock faible" : "Disponible",
        "Date d'ajout": format(new Date(item.createdAt), "dd/MM/yyyy HH:mm", { locale: fr }),
        "Dernière modification": item.updatedAt 
          ? format(new Date(item.updatedAt), "dd/MM/yyyy HH:mm", { locale: fr })
          : "—",
      }));
      ...
      let filename = "inventaire_hospital";
      if (startDate && endDate) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Sélectionnez une période pour exporter les données d'inventaire. Laissez vide pour tout exporter.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="flex items-center justify-between text-sm text-gray-500">
            <span>
              {data.length} article(s) disponibles
            </span>
            <button
