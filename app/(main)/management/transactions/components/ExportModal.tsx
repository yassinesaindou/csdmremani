  ...
  const handleExport = () => {
    ...
    try {
      ...
      const excelData = filteredData.map((item) => ({
        ...
        "Type": item.type === 'income' ? 'Recette' : 'Dépense',
        "Motif": item.reason || "Non spécifié",
        "Montant (KMF)": item.amount || 0,
        "Département": item.department_name || "Général",
        "Créé par": item.created_by_name || "Inconnu",
      ...
      let filename = "transactions_financieres";
      if (startDate && endDate) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Sélectionnez une période pour exporter les transactions. Laissez vide pour tout exporter.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="flex items-center justify-between text-sm text-gray-500">
            <span>
              {data.length} transaction(s) disponibles
            </span>
            <button
