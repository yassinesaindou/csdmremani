...
import { MaternityAppointment } from "../types";
...
export function ExportModal({ open, onOpenChange, data }: ExportModalProps) {
  ...
  // Helper function to translate status to French (matches your APPOINTMENT_STATUSES)
  ...
  const handleExport = () => {
    ...
    try {
      ...
      const excelData = filteredData.map((item) => ({
        "ID Rendez-vous": item.appointmentId,
        "Date Rendez-vous": item.appointmentDate 
          ? format(new Date(item.appointmentDate), "dd/MM/yyyy HH:mm", { locale: fr })
          : "—",
        "Raison du rendez-vous": item.appointmentReason || "—",
        "Statut": getStatusLabel(item.status),
        "Patient": item.patientName || "—",
        "Adresse": item.patientAddress || "—",
        "Téléphone": item.patientPhoneNumber || "—",
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Rendez-vous Maternité");

      // Generate filename with date range
      let filename = "rendez_vous_maternite";
      if (startDate && endDate) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Sélectionnez une période pour exporter les rendez-vous. Laissez vide pour tout exporter.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="flex items-center justify-between text-sm text-gray-500">
            <span>
              {data.length} rendez-vous disponibles
            </span>
            <button
