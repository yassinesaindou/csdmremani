...
interface DiagnosticsComboBoxProps {
  value: string;
  onChange: (diagnostic: string) => void;
  placeholder?: string;
}

export function DiagnosticsComboBox({ 
  value, 
  onChange, 
  placeholder = "Ajouter un diagnostic..." 
}: DiagnosticsComboBoxProps) {
  ...
  const handleAddNew = () => {
    if (search.trim() && !diagnosticsList.includes(search.trim())) {
                ...
                {filteredDiagnostics.map((diag) => (
                  ...
                  >
                    <Check
                      className={cn(
                        "mr-2 h-4 w-4",
                        value === diag ? "opacity-100" : "opacity-0"
                      )}
                    />
                    {diag}
                  </CommandItem>
