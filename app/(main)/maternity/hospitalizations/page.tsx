...
export default function MaternityHospitalizationsPage() {
  const [hospitalizations, setHospitalizations] = useState<MaternityHospitalization[]>([]);
  const [loading, setLoading] = useState(true);
  const [formModalOpen, setFormModalOpen] = useState(false);
  const [exportModalOpen, setExportModalOpen] = useState(false);
  const [editingHospitalization, setEditingHospitalization] = useState<MaternityHospitalization | null>(null);
  ...
  async function loadHospitalizations() {
    ...
    let query = supabase
      .from("hospitalization_maternity")
      .select("*")
    ...
    if (emergency !== "all") query = query.eq("isEmergency", emergency === "true");
    ...
    if (leaveStatus !== "all") {
      if (leaveStatus === "active") {
        ...
        query = query.eq("leave_authorized", false)
  ...
  const handleEditClick = (hospitalization: MaternityHospitalization) => {
  ...
  const handleRowClick = (hospitalization: MaternityHospitalization) => {
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Hospitalisations Maternité</h1>
          <p className="text-gray-600 mt-1">
            Gérer les hospitalisations du département de maternité
          </p>
        </div>
      ...
      <div className="bg-white rounded-lg shadow border">
        {loading ? (
          <div className="flex justify-center items-center h-64">
            ...
            <span className="ml-3 text-gray-600">Chargement des hospitalisations...</span>
          </div>
        ) : (
