...
export function ExportModal({ open, onOpenChange, data }: ExportModalProps) {
  ...
  const getLeaveStatusLabel = (
    hospitalization: MedicineHospitalization
  ): string => {
  ...
  const handleExport = () => {
    ...
    try {
      ...
      const stats = {
        ...
        diedBefore48h: filteredData.filter((item) => item.leave_diedBefore48h)
          .length,
        diedAfter48h: filteredData.filter((item) => item.leave_diedAfter48h)
          .length,
        withLeavingDate: filteredData.filter((item) => item.leavingDate).length,
      };

      // Summary data
      const summaryData = [
        ["STATISTIQUES DES HOSPITALISATIONS - MÉDECINE"],
        [],
        ["TOTAL", "VALEUR"],
        ...
        ["Avec date de sortie renseignée", stats.withLeavingDate],
        [],
        [
          "TOTAL SORTIES",
          stats.authorized +
      ...
      wsDetailed["!cols"] = Array(
        Object.keys(detailedData[0] || {}).length
      ).fill({ wch: maxWidth });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Sélectionnez une période pour exporter les hospitalisations. Laissez
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="flex items-center justify-between text-sm text-gray-500">
            <span>{data.length} hospitalisation(s) disponibles</span>
            <button
