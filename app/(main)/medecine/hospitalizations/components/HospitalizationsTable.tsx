...
import { CheckCircle, XCircle, AlertTriangle, MoreHorizontal, Pencil, Trash2, Eye, Calendar } from "lucide-react";
...
interface HospitalizationsTableProps {
  data: MedicineHospitalization[];
  onRowClick: (hospitalization: MedicineHospitalization) => void;
  onEdit: (hospitalization: MedicineHospitalization) => void;
  onRefresh: () => void;
}

export const columns = (onEdit: (hospitalization: MedicineHospitalization) => void, onRefresh: () => void): ColumnDef<MedicineHospitalization>[] => [
  {
  ...
  },
  {
    ...
    header: "Date Sortie",
    cell: ({ row }) => {
      const leavingDate = row.original.leavingDate;
      return (
        <div className="text-sm">
          {leavingDate ? (
            <>
              {format(new Date(leavingDate), "dd/MM/yy", { locale: fr })}
              <div className="text-xs text-gray-500">
                {format(new Date(leavingDate), "HH:mm", { locale: fr })}
              </div>
            </>
          ) : (
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem asChild>
              <a href={`/medecine/hospitalizations/${hospitalization.hospitalizationId}`}>
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={() => router.push(`/medecine/hospitalizations/${row.original.hospitalizationId}`)}
